    ...
    for (auto dim : dims) {
        ...
        for (auto count : counts) {
            ...
            RaBitQuantizer<MetricType::METRIC_TYPE_L2SQR> quantizer(
                dim, pca_dim, num_bits_per_dim, false, allocator.get());
...
TEST_CASE("RaBitQ Basic Test FHT", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
        uint64_t pca_dim = dim;
        if (dim >= 1500) {
        ...
        for (auto count : counts) {
            ...
            auto vecs = fixtures::generate_vectors(count, dim);
            RaBitQuantizer<MetricType::METRIC_TYPE_L2SQR> quantizer(
                dim, pca_dim, num_bits_per_dim, true, allocator.get());
...
TEST_CASE("RaBitQ Encode and Decode ROM", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
        for (auto count : counts) {
            auto allocator = SafeAllocator::FactoryDefaultAllocator();
            RaBitQuantizer<MetricType::METRIC_TYPE_L2SQR> quantizer(
                dim, dim, num_bits_per_dim, false, allocator.get());
...
TEST_CASE("RaBitQ Encode and Decode FHT", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
        for (auto count : counts) {
            auto allocator = SafeAllocator::FactoryDefaultAllocator();
            RaBitQuantizer<MetricType::METRIC_TYPE_L2SQR> quantizer(
                dim, dim, num_bits_per_dim, true, allocator.get());
...
TEST_CASE("RaBitQ Compute ROM", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
...
TEST_CASE("RaBitQ Compute FHT", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
        float numeric_error = 0.01 / std::sqrt(dim) * dim;
        float related_error = 0.05f;
...
TEST_CASE("RaBitQ Serialize and Deserialize ROM", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
        float numeric_error = 0.01 / std::sqrt(dim) * dim;
        float related_error = 0.05f;
...
TEST_CASE("RaBitQ Serialize and Deserialize FHT", "[ut][RaBitQuantizer]") {
    auto num_bits_per_dim = GENERATE(4, 32);
    for (auto dim : dims) {
        float numeric_error = 0.01 / std::sqrt(dim) * dim;
        float related_error = 0.05f;
...
TEST_CASE("RaBitQ Query SQ4 Transform ROM", "[ut][RaBitQuantizer]") {
    int dim = 5;
    uint64_t num_bits_per_dim_query = 4;
    auto allocator = SafeAllocator::FactoryDefaultAllocator();

    RaBitQuantizer<MetricType::METRIC_TYPE_L2SQR> quantizer(
        dim, dim, num_bits_per_dim_query, false, allocator.get());
...
TEST_CASE("RaBitQ Query SQ4 Transform FHT", "[ut][RaBitQuantizer]") {
    int dim = 5;
    uint64_t num_bits_per_dim_query = 4;
    auto allocator = SafeAllocator::FactoryDefaultAllocator();

    RaBitQuantizer<MetricType::METRIC_TYPE_L2SQR> quantizer(
        dim, dim, num_bits_per_dim_query, true, allocator.get());
